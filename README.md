...
HE nuclei `.npy` と fluorescence nuclei GeoJSON を使う特殊座標系 workflow です。world-µm 座標、Y-flip、ICP、fine center-snap warp、Jacobian QC を扱うため、他の workflow とは独立して維持します。

| 項目 | 内容 |
| --- | --- |
| 入力 | HE nuclei centers `.npy`, fluorescence nuclei GeoJSON, optional HE image |
| 主な処理 | GeoJSON centroid loading, HE point loading, X/Y-flip candidate selection, similarity ICP, affine ICP, fine center-snap warp, Jacobian QC, world-µm scatter QC |
| 出力 | transformed HE centers CSV, HE-GeoJSON transform summary JSON, affine/fine scatter QC PNG, warped HE image PNG |
...
- 座標系によって X/Y flip が必要になるため、flip なし / ありを比較する
